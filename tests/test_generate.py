...
def test_generate_data_with_hypothesis(tmpdir):
    ...
    generated_data = generate.generate_data(
        requested_types, current_size, startsize, level, position, requested_oath
    )
    assert generated_data is not None
...
def test_generate_data_with_gen_func(generate_int_test):
    ...
    generated_data = generate.generate_data(
        requested_types, current_size, startsize, level, position, path, gen_fuc
    )
    assert generated_data is not None
...
def test_detect_level_and_position(tmpdir):
    ...
    path = tmpdir.mkdir("sub").join("hello.txt")
    path.write(
        '[{"type": "array", "items": [{"type": "number"}, {"type": "number"}], "maxItems": 0, "minItems": 0},{"type": "array", "items": [{"type": "number"}, {"type": "number"}], "maxItems": 0, "minItems": 0}]'  # pylint: disable=C0301
    )
    size = "50"
    level = 1
    startsize = 25
...
def test_detect_level_only(tmpdir):
    ...
    position = [0, 0]
    strategy = generate.generate_experiment_strategy(path, size, startsize, level, position)
    assert str(strategy[0]) == "lists(just(50), max_size=25, min_size=25)"
